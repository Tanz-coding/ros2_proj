...
## Campus Delivery 改造（MVP）

本仓库已新增“校园快递投递任务”能力，包含：

- 多站点自动投递（Nav2 `NavigateToPose`）
- `/scan` 触发让行：近距离障碍暂停，恢复后继续
- 到站自动拍照（`/camera/image_raw`）
- 到站 CSV 日志（状态、耗时、最近障碍距离、照片路径）

### 新增文件

- `autonomous_tb/campus_delivery_node.py`
- `autonomous_tb/yield_controller_node.py`
- `launch/campus_delivery.launch.py`
- `config/campus_delivery_params.yaml`
...
2) 一键启动（Gazebo + Nav2 + 任务节点）

```bash
ros2 launch autonomous_tb campus_delivery.launch.py
```

3) 在 RViz 里设置初始位姿（`2D Pose Estimate`），随后任务节点会按 `waypoints` 自动投递。

### 参数配置

在 `config/campus_delivery_params.yaml` 中修改：

- `waypoints`: 每个站点的 `name/x/y/yaw`
- `stop_distance` / `resume_distance`: 让行阈值
- `output_dir`: 照片与 CSV 输出目录
- `image_topic`: 相机话题（默认 `/camera/image_raw`）
